...
class PromptService:
    ...
    async def submit_for_review(
        ...
        try:
            ...
            _env = (
                ...
                .filter(Environment.id == prompt.environment_id)
                .first()
                if prompt
                else None
            )
    ...
    async def approve_version(
        ...
        try:
            ...
            if v.proposed_by_id:
                requester = (
                    self.db.query(User).filter(User.id == v.proposed_by_id).first()
                )
                env = (
                    self.db.query(Environment)
                    .filter(Environment.id == prompt.environment_id)
    ...
    def create_prompt(
        ...
        if (
            ...
            .filter(Prompt.environment_id == env_id, Prompt.key == key)
            .first()
        ):
            raise HTTPException(
                status_code=409, detail="Key exists in this environment"
            )
        ...
        v = PromptVersion(
            ...
            approval_note=(
                f"Policy Check: {len(risks)} risks detected."
                if risks
                else "Policy Check: Pass."
            ),
        )
        self.db.add(v)
    ...
    def create_version(
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    raise HTTPException(
                        status_code=409, detail="Concurrent version creation conflict"
                    )
                continue
    ...
    async def rollback_prompt(
        ...
        for attempt in range(3):
            try:
                ...
                rollback_v = PromptVersion(
                    ...
                    parent_content=(
                        prompt.live_version.content if prompt.live_version else None
                    ),
                )
        ...
        self._log_action(
            ...
            resource_type="version",
            resource_id=rollback_v.id,
            extra={
                "rolled_back_to": target.version_num,
                "new_version": vnum,
                "prompt_key": prompt.key,
            },
        )
    ...
    async def promote_prompt(
        ...
        if not source_prompt or not source_prompt.live_version_id:
            raise HTTPException(
                status_code=404, detail="Source prompt has no live version to promote"
            )

        live_v = (
            self.db.query(PromptVersion)
            .filter(PromptVersion.id == source_prompt.live_version_id)
            .first()
        )
        ...
        target_prompt = (
            self.db.query(Prompt)
            .filter(
                Prompt.environment_id == target_env_id, Prompt.key == source_prompt.key
            )
            .first()
        )
        ...
        self._log_action(
            ...
            resource_type="prompt",
            resource_id=target_prompt.id,
            extra={
                "from_prompt_id": source_prompt.id,
    ...
    def _get_next_version_num(self, prompt_id: str) -> int:
        result = (
            self.db.query(func.max(PromptVersion.version_num))
