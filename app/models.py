...
class User(Base):
    ...
    memberships = relationship(
        "OrgMember", back_populates="user", foreign_keys="OrgMember.user_id"
    )
...
class Environment(Base):
    ...
    prompts = relationship(
        "Prompt", back_populates="environment", cascade="all, delete-orphan"
    )
    api_keys = relationship(
        "ApiKey", back_populates="environment", cascade="all, delete-orphan"
    )
...
class EvalResult(Base):
    ...
    id = Column(String, primary_key=True, default=_uuid)
    version_id = Column(
        String, ForeignKey("prompt_versions.id", ondelete="CASCADE"), nullable=False
    )
    org_id = Column(
        String, ForeignKey("organisations.id", ondelete="CASCADE"), nullable=False
    )
