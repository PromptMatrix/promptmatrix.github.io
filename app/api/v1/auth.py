...
async def register(body: RegisterIn, db: Session = Depends(get_db)):
    ...
    org_name = body.org_name or f"{body.full_name or body.email.split('@')[0]}'s Workspace"
    org = Organisation(name=org_name, slug=_unique_slug(_slugify(org_name), db), plan="free")
    db.add(org)
    db.flush()
    ...
    # Welcome email — awaited directly. Timeout=5s, failure logged not raised.
    try:
