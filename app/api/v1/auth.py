...
async def register(body: RegisterIn, db: Session = Depends(get_db)):
    ...
    org = Organisation(name="PromptMatrix", slug=_unique_slug("promptmatrix", db), plan="local")
    db.add(org)
    db.flush()
    ...
    return {**tokens, "user": {"id": user.id, "email": user.email, "full_name": user.full_name},
            "active_org": {"id": org.id, "name": org.name, "slug": org.slug, "plan": org.plan, "role": member.role}}
