...
class RunEvalIn(BaseModel):
    ...
    api_key: str = ""
    test_input: str = ""
    eval_type: str = "llm_judge"   # llm_judge | rule_based
...
# ── Rule-based eval (no LLM key needed) ─────────────────────────

def _rule_based_eval(content: str) -> dict:
    """Fast heuristic eval. No API key required."""
    score_map = {}
    ...
    # Instruction clarity (has imperative verbs)
...
async def _llm_eval(content: str, test_input: str, provider: str, model: str, api_key: str) -> dict:
    """Run LLM-as-judge eval. Returns same shape as rule_based_eval."""
    import time
    import httpx

    judge_prompt = f"""You are an expert prompt quality evaluator. Evaluate the following system prompt.
...
Test Input (what a user would send): {test_input or "(not provided)"}

Score each criterion from 0.0 to 10.0:
- clarity: Is the role and task clearly defined?
- specificity: Does it give concrete instructions vs vague guidance?
- safety: Does it include appropriate guardrails?
- completeness: Does it cover edge cases and failure modes?
- tone_consistency: Is the tone consistent and appropriate?

Respond ONLY with valid JSON, no markdown:
{{"clarity": 8.5, "specificity": 7.0, "safety": 9.0, "completeness": 7.5, "tone_consistency": 8.0, "strengths": ["list"], "issues": ["list"]}}"""
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            all_headers = {**headers, **cfg["headers"]}
            resp = await client.post(cfg["url"], headers=all_headers, json=cfg["body"])
        ...
        criteria = {k: float(v) for k, v in data.items()
                    if k not in ("strengths", "issues") and isinstance(v, (int, float))}
        ...
        raise HTTPException(status_code=502, detail=f"LLM eval failed: {str(e)}")
...
async def run_eval(
    ...
    # BUG FIX: two separate queries instead of nested .first().attribute
    ...
    if body.eval_type == "rule_based" or body.provider == "rule_based":
        ...
        if not body.api_key or body.api_key == "none":
            raise HTTPException(status_code=400, detail="API key required for LLM eval")
        result = await _llm_eval(v.content, body.test_input, body.provider, body.model, body.api_key)

    # Persist eval result to version
...
async def save_eval_key(
    ...
    db.add(k)
    db.commit()
    return {"message": "Key saved (encrypted)", "id": k.id}
