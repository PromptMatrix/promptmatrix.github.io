...
async def export_workspace(
    ...
    for p in projects:
        ...
        for e in envs:
            ...
            for pr in prompts:
                versions = db.query(PromptVersion).filter(PromptVersion.prompt_id == pr.id).all()
                pr_data = {
                    "key": pr.key,
        ...
        for e_data in p_data.get("environments", []):
            ...
            for pr_data in e_data.get("prompts", []):
                ...
                prompt = db.query(Prompt).filter(
                    Prompt.environment_id == env.id,
                    Prompt.key == pr_data["key"]
                ).first()
                if not prompt:
                    prompt = Prompt(
                ...
                for v_data in pr_data.get("versions", []):
                    ...
                    existing_v = db.query(PromptVersion).filter(
