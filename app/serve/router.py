...
from app.models import (ApiKey, Environment, Organisation, Project, Prompt,
                        PromptVersion, ServeEvent)
from app.serve.cache import (cache_key, cache_prompt, check_rate_limit,
                             get_cached_key, get_cached_prompt)
...
async def _resolve_prompt(env_id, prompt_key):
    ...
    try:
        prompt = (
            db.query(Prompt)
            .filter(Prompt.environment_id == env_id, Prompt.key == prompt_key)
            .first()
        )
        if not prompt or not prompt.live_version_id:
            raise HTTPException(
                status_code=404, detail="Prompt not found or has no live version"
            )
...
async def serve_prompt(
    ...
    if not authorization and settings.app_env == "development":
        ...
        try:
            env = (
                db.query(Environment).filter(Environment.name == "development").first()
                or db.query(Environment).first()
            )
            project = db.query(Project).filter(Project.id == env.project_id).first()
            org = (
                db.query(Organisation).filter(Organisation.id == project.org_id).first()
            )
    ...
    content, unfilled = _substitute_variables(
        content_data["content"], vars, query_params
    )
