...
def test_create_prompt_rejects_invalid_key_format(client, db):
    ...
    for bad_key in ["Has Spaces", "UPPERCASE", "slash/key", "x" * 201]:
        ...
        assert r.status_code == 422, f"Expected 422 for key: {bad_key!r}, got {r.status_code}"
...
def test_full_approval_workflow(client, db):
    ...
    # 2. Create v2
    r = client.post(f"/api/v1/prompts/{prompt_id}/versions", json={
    ...
    # 3. Submit for review
    r = client.post(f"/api/v1/prompts/{prompt_id}/versions/{v2_id}/submit",
                    json={"note": "please review"}, headers=hdrs)
    assert r.status_code == 200
    ...
    r = client.post(f"/api/v1/prompts/{prompt_id}/versions/{v2_id}/approve",
...
def test_approve_archives_previous_live(client, db):
    ...
    r = client.post(f"/api/v1/prompts/{prompt.id}/versions", json={
        "content": "Updated content.", "commit_message": "v2",
    }, headers=hdrs)
    v2_id = r.json()["version"]["id"]
...
def test_reject_version(client, db):
    ...
    r = client.post(f"/api/v1/prompts/{prompt.id}/versions", json={
        "content": "Bad content.", "commit_message": "draft",
    }, headers=hdrs)
    v_id = r.json()["version"]["id"]

    client.post(f"/api/v1/prompts/{prompt.id}/versions/{v_id}/submit", json={}, headers=hdrs)

    r = client.post(f"/api/v1/prompts/{prompt.id}/versions/{v_id}/reject",
                    json={"reason": "off brand"}, headers=hdrs)
    assert r.status_code == 200
...
def test_rollback_creates_new_approved_version(client, db):
    ...
    # Approve v2 over it
    r = client.post(f"/api/v1/prompts/{prompt.id}/versions", json={
        "content": "New content.", "commit_message": "v2",
    }, headers=hdrs)
    v2_id = r.json()["version"]["id"]
    client.post(f"/api/v1/prompts/{prompt.id}/versions/{v2_id}/submit", json={}, headers=hdrs)
    client.post(f"/api/v1/prompts/{prompt.id}/versions/{v2_id}/approve", json={}, headers=hdrs)
...
def test_pending_review_appears_in_approval_queue(client, db):
    ...
    r = client.post(f"/api/v1/prompts/{prompt.id}/versions", json={
        "content": "Needs review.", "commit_message": "v2",
    }, headers=hdrs)
    v_id = r.json()["version"]["id"]
    client.post(f"/api/v1/prompts/{prompt.id}/versions/{v_id}/submit",
                json={"note": "check this"}, headers=hdrs)
...
def test_approving_clears_from_approval_queue(client, db):
    ...
    v_id = r.json()["version"]["id"]
    client.post(f"/api/v1/prompts/{prompt.id}/versions/{v_id}/submit", json={}, headers=hdrs)
    client.post(f"/api/v1/prompts/{prompt.id}/versions/{v_id}/approve", json={}, headers=hdrs)
