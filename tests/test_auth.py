...
def test_register_creates_workspace(client):
    ...
    assert (
        data["active_org"]["name"] == "PromptMatrix"
    )  # OSS: workspace name is always PromptMatrix
...
def test_register_rejects_duplicate_email(client):
    ...
    assert r.status_code in (
        403,
        409,
    )  # OSS: 403=locked after first user; 409=duplicate email
...
def test_refresh_rejects_access_token_as_refresh(client, db):
    ...
    r = client.post(
        "/api/v1/auth/refresh",
        json={"refresh_token": login["access_token"]},  # wrong — passing access token
    )
    assert r.status_code == 401
