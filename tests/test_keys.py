...
def test_revoke_key_blocks_serve(client, db):
    ...
    # Works before revoke
    r = client.get(
        "/pm/serve/assistant.system", headers={"Authorization": f"Bearer {full_key}"}
    )
    assert r.status_code == 200
...
def test_rotate_key_old_key_blocked_new_works(client, db):
    ...
    # New key works
    r = client.get(
        "/pm/serve/assistant.system", headers={"Authorization": f"Bearer {new_key}"}
    )
    assert r.status_code == 200
...
def test_viewer_cannot_create_key(client, db):
    ...
    r = client.post(
        "/api/v1/keys",
        json={"environment_id": env.id, "name": "test-key"},
        headers=viewer_hdrs,
    )
    assert (
        r.status_code == 403
    ), f"Expected 403 for viewer creating key, got {r.status_code}"
...
def test_key_prefix_format(client, db):
    ...
    assert key.startswith(
        "pm_live_"
    ), f"Production key must start with pm_live_, got: {key[:20]}"
